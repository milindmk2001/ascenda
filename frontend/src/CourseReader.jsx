...
export default function CourseReader({ subject, onBack }) {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] w-full bg-slate-950 text-slate-100 overflow-hidden">
      ...
      {/* RENDER SYSTEM SWITCH PANELS */}
      <div className="flex-1 flex overflow-hidden">
        {lessonLoading ? (
          <div className="text-xs font-mono text-slate-500 animate-pulse m-auto">Loading layout context...</div>
        ) : coreContent === "VISUAL_MODE_ACTIVE" && visualLessonPayload ? (
          <VisualLesson lessonPayload={visualLessonPayload} />
        ) : (
          <div className="flex-grow grid grid-cols-2 overflow-hidden bg-slate-900/20">
            {/* LEFT COLUMN: Legacy Markdown Panel */}
            <div className="p-6 overflow-y-auto border-r border-slate-900">
              ...
              <div className="prose prose-invert max-w-none text-slate-300">
                <ReactMarkdown remarkPlugins={[remarkMath]} rehypePlugins={[rehypeKatex]}>
                  {legacyExplanation || "*Select a specific core lesson concept token from the sidebar hierarchy navigation to spin up content views.*"}
                </ReactMarkdown>
              </div>
            </div>

            {/* RIGHT COLUMN: Socratic Text Flow */}
